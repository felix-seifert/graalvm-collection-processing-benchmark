...
def get_no_of_executions(df: DataFrame):
    execution_columns = list(filter(lambda c: c.startswith('T'), df.columns))
    return len(execution_columns)
...
no_of_executions = get_no_of_executions(list_processing_df)
